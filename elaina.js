require('./settings')
const { default: xylaaConnect, useSingleFileAuthState, DisconnectReason, fetchLatestBaileysVersion, generateForwardMessageContent, prepareWAMessageMedia, generateWAMessageFromContent, generateMessageID, downloadContentFromMessage, makeInMemoryStore, jidDecode, proto } = require("@adiwajshing/baileys")
...
async function startxylaa() {
    const xylaa = xylaaConnect({
    ...
    xylaa.ev.on('messages.upsert', async chatUpdate => {
        ...
        require("./elainav4")(xylaa, m, chatUpdate, store)
        } catch (err) {
            console.log(err)
        }
    })
    ...
    xylaa.ev.on('group-participants.update', async (anu) => {
        ...
        try {
            ...
            for (let num of participants) {
                ...
                try {
                    ppuser = await xylaa.profilePictureUrl(num, 'image')
                } catch {
                ...
                //Get Profile Picture Group\\
                try {
                    ppgroup = await xylaa.profilePictureUrl(anu.id, 'image')
                } catch {
...
xylaa.sendMessage(anu.id, buttonMessage, {quoted:unicorndoc})
...
xylaa.sendMessage(anu.id, buttonMessage, {quoted:unicorndoc})
    ...
    xylaa.ev.on('connection.update', async (update) => {
